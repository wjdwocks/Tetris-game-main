...
public class ScoreBoard2 extends JPanel implements KeyListener {
    ...
    public ScoreBoard2() {
        ...
        title.setFont(new Font("Arial", Font.BOLD, Main.SCREEN_WIDTH[1] / 40)); // 폰트 설정
        title.setForeground(Color.BLACK); // 텍스트 색상 설정
        title.setBounds(Main.SCREEN_WIDTH[1]/2 - 200, Main.SCREEN_HEIGHT[1] / 20, 400, 50); // 위치와 크기 설정
        ...
        keyMessage.setBounds(Main.SCREEN_WIDTH[1]/4, Main.SCREEN_HEIGHT[1] / 7, 800, Main.SCREEN_WIDTH[1]/13); // 위치와 크기 설정
    ...
    private void addMenuItem(String text, int x, int y) {
        ...
        menuItem.setBounds(x, y, Main.SCREEN_WIDTH[1] / 2, Main.SCREEN_HEIGHT[1] / 12); // 위치와 크기 설정
